...
### Removed
- 删除底部、地图和经营页的手动“+15 分钟 / +1 小时 / +6 小时”时间跳转入口。
- 删除二手车直接购买、单按钮检测和浏览器弹窗议价的旧流程。

### Added
- 新增自动游戏时钟，默认按 1× 连续推进，并提供暂停、1×、3×、10× 四档控制；应用退到后台时不会补算离屏时间。
- 线路卡新增正式道路总里程和按道路限速估算的行程时间；调度班次新增正式距离、计划耗时和预计到达信息。
- 新增二手车详情面板，展示卖方里程、车况、事故与过户披露。
- 新增基础检测与深度检测，正式展示核验里程、机械/车身状态、事故痕迹和披露差异警告。
- 新增页内正式报价表单，接受价和最终购买价格继续读取同一车辆市场状态。

### Changed
- 游戏时间改由唯一模拟时钟持续驱动，班次上客、发车、行驶、到站与财务结算不再依赖玩家手动点击。
- Android / package 版本升级至 0.20.8，GAME_VERSION 更新为 0.20.8-stage20-automatic-operations。

### Validation
- 新增自动时间与路线距离/耗时展示专项测试。
- 扩展调度中心应用测试，锁定班次距离和计划时长来自正式道路与排班读模型。
- 更新车辆市场展示测试，锁定二手车分级检测与页内议价流程。

## 0.20.7-stage20-fleet-management

### Removed
- 删除车队页直接暴露车辆内部 ID、英文车型级别和英文状态的旧卡片。
- 删除新车点击后绕过配置系统直接购买标准车的旧入口。

### Added
- 新增正式车队管理页，展示品牌、车系、车型、年款、能源、座位、里程、任务与中文运营状态。
- 新增车辆详情面板，集中展示动力、制动、轮胎、车身车况以及保险、年检、保养提醒。
- 新增当前线路、当前司机、下一任务与地图定位，车辆位置继续读取正式地图、班次和站点数据。
- 新增新车选装面板，座位、能源容量、行李舱、舒适度和成交价随正式选装规则实时联动。
- 新增已购车辆配置名称与选装项目展示；新车选装和二手车检测、议价、购买保持独立流程。
- 调度车辆 DTO 新增正式车型身份、配置、车身车况、合规到期时间和运营任务信息。

### Changed
- 车队总览新增规模、可用、运营中和待处理四项指标。
- Android / package 版本升级至 0.20.7，GAME_VERSION 更新为 0.20.7-stage20-fleet-management。

### Validation
- 新增车队管理展示专项测试，锁定中文状态、车辆详情、车况合规、地图定位、新车选装和新旧车分流。
- 扩展调度中心应用测试，验证车辆档案、车身车况、合规到期时间和下一运营任务来自正式读模型。

## 0.20.6-stage19-dispatch-center

### Removed
- 删除调度页最多堆叠 80 张英文状态班次卡片的旧展示方式。
- 删除车辆、司机、班次状态直接暴露内部英文枚举的临时界面。

### Added
- 新增按线路分组的今日班次时间轴，统一显示计划时间、运营状态、车辆、司机、载客量与预计到达时间。
- 调度查询 DTO 新增正式在途载客数与车辆实际座位数，支持选装后容量差异，不由 UI 猜测。
- 新增全部、进行中、待发、异常、已完成五种调度筛选，以及资源缺口和未完成支援任务区域。
- 新增班次地图追踪，可聚焦正式线路并高亮当前运行车辆；未发车班次聚焦对应线路。
- 新增调度计划版本、提交状态和重新排班入口，所有操作继续调用正式 Operations 命令。

### Changed
- 调度总览改为展示进行中、异常/延误、可用车辆、可用司机、支援任务和资源缺口。
- Android / package 版本升级至 0.20.6，GAME_VERSION 更新为 0.20.6-stage19-dispatch-center。

### Validation
- 新增调度中心展示专项测试，锁定线路分组、状态中文化、异常筛选、地图追踪和正式载客字段。
...
### Removed
- 删除每条运营线路分别着色、分别横向偏移、分别强制显示编号的旧叠加方式，避免线路增长后形成彩色蜘蛛网。
- 删除地名找不到空位时仍强制绘制的回退逻辑，标签不再互相覆盖。

### Added
- 新增运营路网汇总层：共享道路只绘制一次，并以线宽表达 1 / 2 / 3 / 4+ 条线路的使用强度。
- 新增地图“线路：汇总/关”和“地名：开/关”快捷控制。
- 线路列表新增“地图查看”，可自动聚焦对应线路并用黄色单线高亮，其他线路保持低干扰汇总显示。

### Changed
- 地名排布与地图 LOD 联动：全国层只显示枢纽，区域层显示枢纽与已解锁城市，近景再逐步显示县级和乡镇站。
- 隐藏级别的站点同步关闭点击热区、锁图标和引导线，防止看不见的站点抢占触控。
- 缩放跨越 LOD 阈值时自动重新计算标签碰撞，不需要刷新页面。
- Android / package 版本升级至 0.20.4，GAME_VERSION 更新为 0.20.4-stage18-map-readability。

### Validation
- 新增地图密度显示专项测试，锁定路网汇总、标签碰撞隐藏、线路聚焦与图层开关。
- 正式道路、线路、车辆位置与里程仍使用唯一 WorldGraph 数据，不引入第二套显示路网。

## 0.20.3-stage18-terrain-fit

### Removed
- 删除东南岛移除后残留在海面上的旧站点坐标、路口坐标与跨海支线几何。
- 删除东南沿海低价值重复连接，停止让多条国省县乡道路挤在同一片海岸。

### Changed
- 青浦、榕海、海宁迁回东南内陆；澜山、云港、南港迁入西北空白区，48 个永久站点 ID、站级和解锁规则保持不变。
- 重接 N104、P209、C310、C312、L905、L906 的正式端点，使西北新增城市形成真实支线，东南路网不再保留跨区拉线。
- 28 个站点/互通/环线路口完成陆地校准，47 段受影响道路依据地形重新寻路并重算正式里程。
- 中央大湖、东部湖群与东南海岸的国道、省道和高速改为绕湖、沿谷地或在窄水面过桥，不再直穿大型水体。
- 河源、永安起步城市外环收紧到陆地区域，避免环线切入湖面。
- Android / package 版本升级至 0.20.3，GAME_VERSION 更新为 0.20.3-stage18-terrain-fit。

### Validation
- 新增底图联动检查：48 个客运站中心不得落入水面。
- 新增高速、国道、省道连续水面跨度检查，超过 12km 即阻止进入 main。
- 新增路网分布守卫：西北普通城市与支线数量不得回退，东南站点密度不得恢复为旧拥挤布局。
- 正式地图仍只通过 world-map.v1.json → WorldGraph 单一链路进入寻路、线路、车辆位置和里程计算。

## 0.20.2-stage18-organic-map

### Removed
- 删除东南海域无城市、无道路、无客运玩法的大岛及其附属岛礁，不再浪费全国地图可视面积。
- 删除城市成排、沿海站点挤成一团的旧坐标排列。

### Changed
- 48 个正式客运站重新做自然化分布：保留永久 ID、站级、解锁与业务引用，只调整世界坐标。
- 南部和东部最拥挤城市群重新拉开间距，任意两个客运站中心间距不低于 50km。
- 194 段正式道路全部重算 polyline 与里程；所有道路至少保留 5 个控制点，高速至少 9 个控制点。
- 高速、国道、省道、县乡道增加不同幅度的非规则弧度，端点仍严格绑定正式 WorldNode。
- 全国级与区域级高速线宽下调，减少粗绿色框线遮挡地形和城市的问题。
- Android / package 版本升级至 0.20.2，GAME_VERSION 更新为 0.20.2-stage18-organic-map。

### Validation
- 新增站点最小间距检查与城市横向排队回退检查。
- 新增全路网最小控制点检查。
- 路网拓扑、道路等级、道路 ID、48 个站点 ID 与正式寻路入口保持唯一且不变。
...
- 所有客运站必须继续保持全图连通。
- Core Check 与 Android APK 构建必须同时通过。
- 收紧中央 H21/H02 互通北侧节点，L951 匝道由约 110.7km 降至约 78.8km，继续满足正式匝道长度上限。
...
### Removed
- 删除城市名称固定放在站点右上方、互相重叠的单一布局。
- 删除所有玩家线路完全重叠成同一条橙线的显示方式。
- 未解锁城市不再以接近已解锁城市的视觉强度占据画面。

### Added
- 新增地图标签碰撞检测与占位盒。
- 城市标签按 hub / city / county / town 优先级自动选择右上、左上、右下、左下、上方或下方位置。
- 标签偏移较大时新增短引线，保持城市点和名称对应关系。
- 玩家线路新增 4 组暖色视觉色阶，并继续与道路等级色分离。
- 多条运营线路共路时新增最多 5 个小幅并行槽位，视觉偏移固定在约 ±4.4px 内。
- 激活线路首末端新增 routeStop 环形标记。
- 线路编号牌新增城市标签避让逻辑，优先尝试路径中部附近多个候选位置。
- 左下图例新增“运营线路”示例。
- 地图 HUD 新增当前激活运营线路数量。

### Changed
- 未解锁城市点与文字进一步降低不透明度。
- 线路牌尺寸、阴影和线路 halo 重新收敛，使其突出但不遮道路细节。
- Android / package 版本升级至 0.19.3。
- GAME_VERSION 更新为 0.19.3-stage17-route-city-labels。

### Validation
- 本版不修改 WorldMap 路网拓扑、城市解锁门槛、PassengerRoute 实际路径或车辆运行状态。
- Core Check 与 APK 构建继续作为正式提交门禁。


## 0.19.2-stage17-road-visual-master

### Removed
- 删除高速、国道、省道全部使用近似同一绿色色相的单调视觉。
- 删除全国视角与近景使用相同高速线宽的表现。
- 全国视角不再显示道路编号牌和高速内部细节。
- 降低道路阴影强度，避免道路像悬浮塑料管。

### Added
- 正式道路色阶：高速深玉绿、国道低饱和朱红、省道灰金绿、县乡道灰绿。
- 所有道路继续使用统一暖白路面，使不同路级有颜色差但仍属于同一地图体系。
- 新增 road-detail 渲染层：仅近景高速/国道显示细节线。
- 新增 LOD 线宽规则：全国/区域/城市群/近景四级视角使用不同高速与国道线宽。
- 全国级高速自动减薄，并隐藏 roadShield。
- 玩家运营线路改为更亮的橙色主线 + 低透明度深橙 halo，与朱红国道仍保持明显区别。
- 地形背景进一步降低山地对比度、略增强河流识别，使路网层次更清楚。

### Changed
- junction 配色同步跟随各道路等级，不再全部使用绿色。
- 城市环线、接驳、匝道和地方接入统一收窄，近景不易糊成一团。
- Android / package 版本升级至 0.19.2。
- GAME_VERSION 更新为 0.19.2-stage17-road-visual-master。

### Validation
- 本版不修改 WorldMap 路网拓扑、道路编号、城市解锁、线路寻路、车辆位置或财务逻辑。
- Core Check 与 APK 构建继续作为正式提交门禁。


## 0.19.1-stage17-map-master-shell

### Removed
- 地图页不再显示占据整行的全局时间推进 footer。
- 删除地图页底部无效留白和“地图 + 大块控制条”分割布局。
- 删除地图 HUD 与缩放控件过小、贴边、缺少层次的工程页布局。

### Added
- 地图页新增动态高度适配：运行时读取 header 与底部 nav 实际高度，地图自动占满中间可用区域。
- 地图页新增 map-active 模式，只有地图页隐藏全局 footer，其他经营页面继续保留原时间控制。
- 时间推进改为地图内悬浮 dock：+15分 / +1小时 / +6小时。
- 地图 HUD 重做为更完整的信息卡片样式。
- 右侧缩放/复位控件扩大点击区域并增加统一悬浮层级。
- 图例改为更紧凑的底部悬浮卡片。
- “新建线路”改为右下角独立主操作按钮。
- 顶部公司信息栏、底部导航栏同步精修圆角、间距、字体权重和阴影。

### Changed
- 地图页使用完整可用屏幕高度，不再依赖固定 176px 估算。
- 地图页切换时通过 openPage() 统一管理 body.map-active，不建立第二套页面入口。
- Android / package 版本升级至 0.19.1。
- GAME_VERSION 更新为 0.19.1-stage17-map-master-shell。

### Validation
- 本版不修改 WorldMap 路网、城市解锁、路线寻路、车辆运行或财务状态。
- Core Check 与 APK 构建继续作为正式提交门禁。


## 0.19.0-stage17-final-map-ui

### Removed
- 删除正式高速骨架中的 H12 / H14 / H22 三组横纵切割式高速。
- 删除“多条近似平行横线 + 多条近似竖线”形成的格子型高速组织。
- 高速不再采用过粗、过饱和的主视觉宽度压住国道和省道。

### Added
- 正式高速骨架固定为 6 条自然走廊：H01 / H02 / H03 / H11 / H13 / H21。
- 34 个既有高速 RoadSegment ID 全部复用并重新编组，没有建立第二套高速实现。
- H01 / H02 / H03 作为三条主要弧形跨区走廊。
- H11 / H13 / H21 作为三条斜向联络走廊，取代规则纵向切割。
- 新增 6 个高速专用中间 junction，用于连续弯曲和非等距走廊。
- 内容测试固定检查 6 条高速走廊的段数：7 / 8 / 7 / 4 / 4 / 4。
- 测试明确禁止 H12 / H14 / H22 重新进入 expressway 正式内容。
- 高速、国道、省道、县乡道路最终视觉宽度重新收口；高速保持最明显但不再压住其他路级。
- 玩家运营线路暖色高亮继续位于道路最上层。

### Changed
- 正式地图仍为 200 个道路段，道路等级数量不变；道路节点从 130 增至 136。
- 高速道路全部保持多控制点自然几何，最长单段低于 520km 风格上限。
- Android / package 版本升级至 0.19.0。
- GAME_VERSION 更新为 0.19.0-stage17-final-map-ui。

### Validation
- 五级道路规模继续保持：高速 34、国道 40、省道 77、县道 10、乡道 39。
- 所有 48 个客运站必须保持路网连通。
- 高速正式代码集合必须精确等于 H01/H02/H03/H11/H13/H21。
- Core Check 与 APK 构建继续作为提交门禁。


## 0.18.9-stage17-road-skin

### Removed
- 删除上一版道路仅靠“绿色粗边 + 白芯”形成的单薄塑料感。
- 删除 junction 只用单层浅色圆点覆盖道路接口的表现。
- 降低山地、丘陵、河流与水体背景对比度，避免抢道路视觉。

### Added
- 道路新增第三层 road-shadow：在道路底部增加低透明度柔和轮廓，不使用高成本模糊滤镜。
- 五级道路重新调整边框宽度、路面宽度和透明度比例。
- 城市环线、接驳线、匝道、地方接入拥有独立细化宽度。
- junction 新增 junctionCasing + junctionPatch 双层融合：先融合道路外边框，再融合暖白路面。
- 主要互通保留轻量 junctionHalo，只作为结构提示，不再显示黑色节点。
- 道路编号牌缩小并调整为更接近导航地图的紧凑比例。
- 地图图例同步采用新道路宽度体系。

### Changed
- 道路本体统一为绿色边框 + 暖白路面；高速、国道、省道通过边框浓度和宽度区分。
- 国道编号牌保留小面积红色，仅用于识别，不再把道路本身染红。
- Android / package 版本升级至 0.18.9。
- GAME_VERSION 更新为 0.18.9-stage17-road-skin。

### Validation
- 本版不修改正式 WorldMap 路网拓扑、道路长度、线路寻路或城市解锁规则。
- Core Check 与 APK 构建继续作为提交门禁。
